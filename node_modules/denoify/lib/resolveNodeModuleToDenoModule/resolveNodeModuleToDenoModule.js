...
exports.getValidImportUrlFactory = (() => {
    ...
    const { getDenoifyOutDir } = (() => {
        const getExplicitDenoifyOutDir = (0, addCache_1.addCache)(async (params) => {
            ...
            const { buildUrl } = buildUrlFactory({ moduleAddress });
            const packageJson = await (0, node_fetch_1.default)(buildUrl(gitTag, "package.json")).then(res => (`${res.status}`.startsWith("2") ? res.text() : undefined), () => undefined);
            if (packageJson === undefined) {
                return undefined;
            }
            const denoifyOut = (_a = JSON.parse(packageJson)["denoify"]) === null || _a === void 0 ? void 0 : _a["out"];
